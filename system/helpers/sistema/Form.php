<?php
	final class Form {
		...
		**/
		 ...
		 * ARQUIVO LISTA
		**/
		final public static function arquivo_lista($cod, $titulo, $arquivos, $ext, $local = 1){
			$lista = '';
			if($arquivos):
				foreach($arquivos as $r):
					$lista .= '
						<li class="arquivo" style="background-image: url('.$r->imagem.')">
							<a class="download" download="'.$r->titulo.'" href="'.$r->arquivo->link.'"><i data-font="&#xf0ed;"></i></a>
			...
			return '
				<div class="bloco_arquivo_lista" data-cod="'.$cod.'" data-diretorio="arquivo" data-ext="'.$ext.'" data-local="'.$local.'">
					<div class="titulo_sub">'.$titulo.'</div>
					<div class="botao"><input type="file"><i data-font="&#xf0ee;"></i> UPLOAD</div>
					<ul class="lista">
						'.$lista.'
		...
		final public static function imagem_lista($app, $cod, $titulo, $valor = null, $diretorio, $ext, $width = '', $height = '', $tipo = 'normal', $retorno = 'css'){
			...
			if($valor):
				foreach($valor as $r):
					...
					$lista .= '<figure '.$css.' data-id="'.$r->id.'">'.$img.'<i class="fechar" data-font="&#xe813;" data-ajuda="Remover imagem"></i><input type="text" placeholder="Nome da imagem" /></figure>';
				endforeach;
			else:
		...
		**/
		final public static function datahora($nome = null, $valor = null, $attr = [], $calendario = false){
			$nome = ($nome != null) ? 'name="'.$nome.'"' : '';
			$valor = ($valor != null) ? Converter::data($valor, 'd/m/Y H:i') : '';
			$placeholder = (isset($attr['placeholder'])) ? $attr['placeholder'] : '00/00/0000 00:00';
			$attr_string = '';
			if(!empty($attr)) foreach($attr as $ind => $val) $attr_string .= ' '.$ind.'="'.$val.'"';
