...
$(function() {
  ...
  $.calendario({
    id: ".bloco_data .botao_data"
  });
  $.calendario({
    id: ".bloco_data .botao_data_hora",
    hora: true
  });
  ...
  $("body").on(
    "change",
    ".input_url_geral select, .input_url_geral input[type=url]",
    function() {
      var bloco = $(this).closest(".input_url_geral");
      $("input:eq(0)", bloco).val(
        $("select", bloco).val() + $("input[type=url]", bloco).val()
      );
    }
  );
  /**
  ...
  var input_tamanho = function(atual, tamanho, input) {
    if ($("#plugins_input_tamanho").size() == 1)
      $("#plugins_input_tamanho").remove();

    $("body").append(
      '<div id="plugins_input_tamanho">' +
        atual +
        " de " +
        tamanho +
        " caracteres</div>"
    ...
    $("#plugins_input_tamanho").css({
      top:
        input.offset().top +
        parseFloat(input.css("height")) +
        parseFloat(input.css("padding-top")) +
        parseFloat(input.css("padding-bottom")) +
        parseFloat(input.css("border-top-width")) +
        parseFloat(input.css("border-bottom-width")),
  ...
  $("body").on("click", "#bloco_cor_mascara", function(e) {
    if ($(e.target).attr("id") == "bloco_cor_mascara")
      $("#bloco_cor_mascara").remove();
  });
  $("body").on("click", "#bloco_cor_mascara li.but_cor_selecionar", function() {
    ...
    $(".input_cor_geral[data-cor-id=" + id + "] .botao_cor").css(
      "background-color",
      cor
    );
...
(function($) {
  $.mapa = function(options) {
    ...
    googlemaps.iniciar = function(
      ...
      googlemaps.mapa = new google.maps.Map(
        document.getElementById(mapa_id),
        options
      );
    ...
    google.maps.event.addDomListener(
      ...
      googlemaps.iniciar(
        ...
        settings.icone,
        settings.draggable,
        settings.input_latitude,
        settings.input_longitude,
        settings.como_chegar
      )
    );
  };
    ...
    $("body").on(
      {
        mouseenter: function() {
          ...
          if (html != "") {
            ...
            var help_width =
              $("#plugins_ajuda").width() +
              parseFloat($("#plugins_ajuda").css("padding-left")) +
              parseFloat($("#plugins_ajuda").css("padding-right")) +
              parseFloat($("#plugins_ajuda").css("border-left-width")) +
              parseFloat($("#plugins_ajuda").css("border-right-width"));
            var div_width =
              $(this).width() +
            ...
            if (div_width >= help_width)
              help_left = div_left + (div_width - help_width) / 2;
            else if (div_width < help_width)
              help_left = div_left - (help_width - div_width) / 2;
            ...
            var help_height =
              $("#plugins_ajuda").height() +
              parseFloat($("#plugins_ajuda").css("padding-top")) +
              parseFloat($("#plugins_ajuda").css("padding-bottom")) +
              parseFloat($("#plugins_ajuda").css("border-top-width")) +
              parseFloat($("#plugins_ajuda").css("border-bottom-width"));
            var div_height =
              $(this).height() +
  ...
  $.textarea = function(options) {
    ...
    if (settings.barra == "completa") {
      var barra =
        "undo redo | bold italic | removeformat | forecolor backcolor | blockquote | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link unlink | image media table | codesample code fullscreen | paste | fontsizeselect";
    } else if (settings.barra == "normal") {
      var barra =
        "undo redo | bold italic | removeformat | forecolor backcolor | blockquote | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link unlink | image media | paste | fontsizeselect";
    } else {
      var barra =
        "bold italic | removeformat | link unlink | paste | fontsizeselect";
    }
    tinymce.init({
      selector: settings.id,
      images_upload_url: LINK + "/upload.php",
      images_upload_base_path: ARQUIVO + "/drive",
      ...
      language: "pt_BR",
      plugins: [
        "advlist autolink lists link image charmap print preview hr anchor pagebreak paste",
        ...
        "template paste textcolor colorpicker textpattern imagetools codesample toc paste",
        "paste"
      ],
      paste_as_text: true,
      toolbar:
        "undo redo | bold italic | removeformat | forecolor backcolor | blockquote | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link unlink | image media table |  fullscreen | fontsizeselect",
      paste_as_text: true
    });
  };
  ...
  $.calendario = function(options) {
    ...
    var semana_longo = getSemana(data.getDay(), 3);
    // Mes
    var mes_longo = getMes(data.getMonth());
    var mes_passado = mes - 1;

    // Número de dias que tem o mês atual
    var dias_quantidade = getDiaUltimo(ano, mes);
    // Número de dias que tem o mês passado
    var dias_quantidade_passado = getDiaUltimo(ano, mes_passado);
    // Primeira semana do mês atual
    var semana_primeira = new Date(ano, mes, 1).getDay();
    // Última semana do mês atual
    var semana_ultimo = new Date(ano, mes, dias_quantidade).getDay();
    // Array com a quantidade de semanas restantes para devidos cálculos
    var array_semana_ultima = new Array(7);
    array_semana_ultima[0] = 6;
    array_semana_ultima[1] = 5;
    array_semana_ultima[2] = 4;
    array_semana_ultima[3] = 3;
    array_semana_ultima[4] = 2;
    array_semana_ultima[5] = 1;
    array_semana_ultima[6] = 0;
    // Quantidade de semanas a mais que terá que ser colocado no bloco
    var semana_quantidade_a_mais = array_semana_ultima[semana_ultimo];

    // Função para pegar o último dia do mês
    function getDiaUltimo(ano, mes) {
      mes++;
      return new Date(ano, mes, 0).getDate();
    }
    // Converte o dia da semana em extenso
    function getSemana(semana, tamanho) {
      var array_semana = new Array(7);
      array_semana[0] = "Domingo";
      array_semana[1] = "Segunda";
      array_semana[2] = "Terça";
      array_semana[3] = "Quarta";
      array_semana[4] = "Quinta";
      array_semana[5] = "Sexta";
      array_semana[6] = "Sábado";

      if (tamanho) {
        return array_semana[semana].substring(0, tamanho);
      } else {
        return array_semana[semana];
      }
    }

    // Converte o mês em extenso
    function getMes(mes, tamanho) {
      var array_mes = new Array(12);
      array_mes[0] = "JANEIRO";
      ...
      if (tamanho) {
        return array_mes[mes].substring(0, tamanho);
      } else {
        return array_mes[mes];
      }
    }

    // Função para pegar a primeira semana do mês
    function getSemanaPrimeiro(ano, mes) {
      return new Date(ano, mes, 1).getDay();
    }

    // Adiciona estrutura HTML
    // if ($("#plugins_calendario_mascara").size() > 0)
    //   $("#plugins_calendario_mascara").remove();

    // $("body").append(
    //   '<div id="plugins_calendario_mascara"><div id="plugins_calendario"><div id="plugins_calendario_header"><div class="plugins_calendario_botoes" id="plugins_calendario_mes_anterior" data-acao="mes_anterior"></div><div class="plugins_calendario_texto" id="plugins_calendario_mes_texto">JUNHO</div><div class="plugins_calendario_botoes" id="plugins_calendario_mes_proximo" data-acao="mes_proximo"></div><div class="plugins_calendario_botoes" id="plugins_calendario_ano_anterior" data-acao="ano_anterior"></div><div class="plugins_calendario_texto" id="plugins_calendario_ano_texto">2016</div><div class="plugins_calendario_botoes" id="plugins_calendario_ano_proximo" data-acao="ano_proximo"></div><div class="plugins_calendario_botoes" id="plugins_calendario_fechar"></div></div><ul class="plugins_calendario_coluna" id="plugins_calendario_semana"><li>' +
    //     getSemana(0, 3) +
    //     "</li><li>" +
    //     getSemana(1, 3) +
    //     "</li><li>" +
    ...
    //     "</li><li>" +
    //     getSemana(6, 3) +
    //     '</li></ul><ul class="plugins_calendario_coluna" id="plugins_calendario_datas"></ul><div id="plugin_calendario_rodape"><input type="text" id="plugins_calendario_hora" onClick="this.select()" data-mascara="hora" value="" placeholder="00:00:00"><div id="plugins_calendario_ok">OK</div><div class="plugins_calendario_agora" id="plugins_calendario_hoje">HOJE</div><div class="plugins_calendario_agora" id="plugins_calendario_agora">AGORA</div></div></div></div>'
    // );

    // Sistema para criar o campo de data
    function getDataInicial(acao) {
      var data_id = $("#plugins_calendario").attr("data-id");
      var data_minimo = $("#plugins_calendario").attr("data-minimo");
      var data_maximo = $("#plugins_calendario").attr("data-maximo");
      var data_hora = $("#plugins_calendario").attr("data-hora");

      if (plugins_calendario_id.indexOf(data_id) == -1)
        plugins_calendario_id.push(data_id);

      var data = "";
      var hora = "";

      // Remover um mês
      if (acao == "mes_anterior") {
        if (mes == 0) {
          mes = 11;
          ano--;
        } else {
        ...
        mes_longo = getMes(mes);
        mes_passado = mes - 1;
      } else if (acao == "mes_proximo") {
        if (mes == 11) {
          mes = 0;
          ano++;
        } else {
        ...
        mes_longo = getMes(mes);
        mes_passado = mes - 1;
      } else if (acao == "ano_anterior") {
        ano--;
      } else if (acao == "ano_proximo") {
        ano++;
      } else if ($(data_id).val()) {
        var data_hora_explode = $(data_id)
          .val()
          .split(" ");

        data = data_hora_explode[0] || "";
        hora = data_hora_explode[1] || "";
        if (!$.isHora(hora)) hora = "";

        // Valida a data
        if ($.isData(data)) {
          var data_explode = data.replace(/[^0-9]/g, "");

          dia = data_explode.substring(0, 2);
          mes = data_explode.substring(2, 4);
          ano = data_explode.substring(4, 8);
          mes--;
          mes_longo = getMes(mes);
        }
      }

      if (data_hora == 1) $("#plugins_calendario_hora").val(hora);
      else $("#plugins_calendario_hora").val("");

      // Coloca novos valores
      dias_quantidade = getDiaUltimo(ano, mes);
      dias_quantidade_passado = getDiaUltimo(ano, mes_passado);
      semana_primeira = new Date(ano, mes, 1).getDay();
      semana_ultimo = new Date(ano, mes, dias_quantidade).getDay();
      semana_quantidade_a_mais = array_semana_ultima[semana_ultimo];

      // Cria lista de datas
      var dias_lista = "";
      dias_mes_ultimo = dias_quantidade_passado - semana_primeira;

      // Datas negativas
      for (i = 0; i < semana_primeira; i++) {
        dias_mes_ultimo++;
        dias_lista +=
          '<li class="plugins_calendario_dias_negativo">' +
          dias_mes_ultimo +
          "</li>";
      }
      // Datas do mes
      for (i = 1; i <= dias_quantidade; i++) {
        var dia_real = i;
        if (i < 10) dia_real = "0" + i;
        var mes_real = parseInt(mes) + 1;
        if (mes_real < 10) mes_real = "0" + mes_real;

        var data_real = dia_real + "/" + mes_real + "/" + ano;
        var data_eua = parseInt("" + ano + mes_real + dia_real);

        if (data_minimo > 0 && data_minimo > data_eua)
          dias_lista +=
            '<li class="plugins_calendario_dias_negativo">' + i + "</li>";
        else if (data_maximo > 0 && data_maximo < data_eua)
          dias_lista +=
            '<li class="plugins_calendario_dias_negativo">' + i + "</li>";
        else if (data == data_real)
          dias_lista +=
            '<li class="plugins_calendario_dias plugins_calendario_dias_hover" data-dia="' +
            data_real +
            '">' +
            i +
            "</li>";
          ...
          dias_lista +=
            '<li class="plugins_calendario_dias" data-dia="' +
            data_real +
            '">' +
            i +
            "</li>";
      ...
      // Datas negativas
      for (i = 1; i <= semana_quantidade_a_mais; i++) {
        dias_lista +=
          '<li class="plugins_calendario_dias_negativo">' + i + "</li>";
      }

      // Adiciona novo mês e ano
      $("#plugins_calendario_mes_texto").text(mes_longo);
      $("#plugins_calendario_ano_texto").text(ano);

      // Adiciona lista da data
      $("#plugins_calendario_datas").html(dias_lista);

      // Posiciona o bloco
      var bloco_width =
        parseFloat($(data_id).width()) +
        parseFloat($(data_id).css("border-left")) +
        parseFloat($(data_id).css("border-right"));
      var bloco_height =
        parseFloat($(data_id).height()) +
        parseFloat($(data_id).css("border-top")) +
        parseFloat($(data_id).css("border-bottom"));

      // var calendario_height = $('#plugins_calendario').height();
      //
      // var bloco_top = $(data_id).offset().top;
      // var bloco_left = $(data_id).offset().left;
      //
      // var scroll_top = $(document).scrollTop();
      // var position = bloco_top-scroll_top;

      // if(acao == 'abrir' && position > ($(window).height() / 2)){
      // 	$('#plugins_calendario').css({
      // 		'top'	: bloco_top-calendario_height,
      // 		'left' 	: bloco_left
      // 	});
      // }else if(acao == 'abrir'){
      // 	$('#plugins_calendario').css({
      // 		'top'	: bloco_top+bloco_height,
      // 		'left' 	: bloco_left
      // 	});
      // }
    }

    // Chama o mapa quando da focus
    $("body").on("click", settings.id, function() {
      $("#plugins_calendario_mascara")
        .show()
        .stop()
        .animate({ opacity: 0 }, 0)
        .animate({ opacity: 1 }, 300);
      // Verifica se vai passar a hora ou não
      if (settings.hora == true) {
        $("#plugins_calendario").attr("data-hora", 1);
        $("#plugins_calendario_hora").show();
        $("#plugins_calendario_agora").show();
        $("#plugins_calendario_ok").show();
        $("#plugins_calendario_hoje").hide();
        $(this).attr("data-pluginscalendario-hora", 1);
      } else {
        $("#plugins_calendario").attr("data-hora", 0);
        $("#plugins_calendario_hora").hide();
        $("#plugins_calendario_agora").hide();
        $("#plugins_calendario_ok").hide();
        $("#plugins_calendario_hoje").show();
        $(this).attr("data-pluginscalendario-hora", 0);
      }

      // Verifica se tem uma data mínima
      var minimo = 0;
      if ($.isData(settings.minimo) || $.isDataHora(settings.minimo)) {
        minimo = settings.minimo;
      } else if ($(settings.minimo).size() == 1) {
        minimo = $(settings.minimo).val();
      }
      if (minimo != 0) {
        var data_minimo = minimo.replace(/[^0-9]/g, "");
        minimo =
          data_minimo.substring(4, 8) +
          data_minimo.substring(2, 4) +
          data_minimo.substring(0, 2);
      }
      $("#plugins_calendario").attr("data-minimo", minimo);
      // Verifica se tem uma data maxima
      var maximo = 0;
      if ($.isData(settings.maximo) || $.isDataHora(settings.maximo)) {
        maximo = settings.maximo;
      } else if ($(settings.maximo).size() == 1) {
        maximo = $(settings.maximo).val();
      }
      if (maximo != 0) {
        var data_maximo = maximo.replace(/[^0-9]/g, "");
        maximo =
          data_maximo.substring(4, 8) +
          data_maximo.substring(2, 4) +
          data_maximo.substring(0, 2);
      }
      $("#plugins_calendario").attr("data-maximo", maximo);

      // Pega o ID do input
      var bloco = $(this).closest(".bloco_data");
      var input_id = $("input", bloco).attr("id");

      if (input_id == undefined) {
        input_id = "div_" + Math.round(Math.random() * 1000000);
        $("input", bloco).attr("id", input_id);
      }
      $("#plugins_calendario").attr("data-id", "#" + input_id);

      getDataInicial("abrir");
      return false;
    });

    // Verifica se a data é válida
    $("body").on("blur", settings.id, function() {
      if ($("#plugins_calendario").is(":visible")) {
        var id = $("#plugins_calendario").attr("data-id");
        var hora = $(id).attr("data-pluginscalendario-hora") || 0;
        if (hora == 1) {
          if (!$.isDataHora($(id).val())) $(id).val("");
        } else {
          if (!$.isData($(id).val())) $(id).val("");
        }
      }
    });

    // Muda o ano e mes do calendario
    $("body").on(
      "click",
      "#plugins_calendario_mes_anterior, #plugins_calendario_mes_proximo, #plugins_calendario_ano_anterior, #plugins_calendario_ano_proximo",
      function() {
    ...
    var calendario_fechar = function() {
      $("#plugins_calendario_mascara")
        .stop()
    ...
    // Fecha o bloco
    $("body").on("click", "#plugins_calendario_fechar", function() {
      if ($("#plugins_calendario_mascara").is(":visible")) {
        calendario_fechar();
      }
    });
    $("body").on("click", "#plugins_calendario_mascara", function(e) {
      if ($(e.target).attr("id") == "plugins_calendario_mascara") {
        calendario_fechar();
      }
    });
    // Da OK quando tiver data
    $("body").on("click", "#plugins_calendario_ok", function() {
      var input = $("#plugins_calendario").attr("data-id");
      var data =
        $("#plugins_calendario .plugins_calendario_dias_hover").attr(
          "data-dia"
        ) || "";
      var hora = $("#plugins_calendario_hora").val();

      if ($.isData(data)) {
        if (!$.isHora(hora)) hora = "00:00:00";
        $(input).val(data + " " + hora);
      }
      calendario_fechar();
    });

    // Ação ao clicar em uma data
    $("body").on("click", ".plugins_calendario_dias", function() {
      var calendario = $(this).closest("#plugins_calendario");
      var input = calendario.attr("data-id");
      var hora = calendario.attr("data-hora");

      $("#plugins_calendario .plugins_calendario_dias").removeClass(
        "plugins_calendario_dias_hover"
      );
      $(this).addClass("plugins_calendario_dias_hover");

      if (hora == 1) {
        var valor = $(this).attr("data-dia");
        var hora = $("#plugins_calendario_hora").val();
        if ($.isData(valor)) {
          if (!$.isHora(hora)) hora = "00:00:00";
          $(input).val(valor + " " + hora);
        }
        $("#plugins_calendario_hora").focus();
      } else {
        var valor = $(this).attr("data-dia");
        $(input).val(valor);
        calendario_fechar();
    ...
    $("body").on("keyup", "#plugins_calendario_hora", function(e) {
      var input = $("#plugins_calendario").attr("data-id");
      var data =
        $("#plugins_calendario .plugins_calendario_dias_hover").attr(
          "data-dia"
      ...
      if ($.isData(data) && $.isHora(hora)) {
        $(input).val(data + " " + hora);
      }
    });

    $("#plugins_calendario").on(
      "click",
      "#plugins_calendario_hoje",
      function() {
        var data = new Date();
        var dia = data.getDate();
        var mes = data.getMonth();
        ...
        if (dia < 10) dia = "0" + dia;
        if (mes < 10) mes = "0" + mes;
        $($("#plugins_calendario").attr("data-id")).val(
          dia + "/" + mes + "/" + ano
        );
        calendario_fechar();
      }
  ...
  $.galeria = function(options) {
    ...
    $(settings.id + " " + settings.botao).click(function() {
      ...
      if (!acao && original.size() == 1) {
        ...
        if (MOBILE)
          $("body").prepend(
            '<div class="plugins_icon" id="plugins_galeria_fechar_mobile"></div>'
          );
        else
          $("body").prepend(
            '<div class="plugins_icon" id="plugins_galeria_fechar"></div>'
          );
        if (!MOBILE && quantidade > 1) {
          $("body").prepend(
            '<div class="plugins_icon" id="plugins_galeria_prev"></div>'
          );
          $("body").prepend(
            '<div class="plugins_icon" id="plugins_galeria_next"></div>'
          );
        }
        $("#plugins_galeria_mascara")
          .stop()
          .animate({ opacity: 1 });
        imagem.animate(
          {
    ...
    function fechar() {
      ...
      if (imagem.size() == 1) {
        ...
        $(
          "#plugins_galeria_fechar, #plugins_galeria_prev, #plugins_galeria_next, #plugins_galeria_fechar_mobile"
        ).remove();
    ...
    $("body").on(
      "click",
      "#plugins_galeria_mascara, #plugins_galeria_fechar, #plugins_galeria_fechar_mobile",
      function() {
        fechar();
      }
    );
    ...
    if (MOBILE) {
      window.onhashchange = function() {
        ...
        if (
          url_atual.indexOf("#galeriamobile") == -1 &&
          $("#plugins_galeria_imagem").size() == 1
        ) {
          fechar();
        }
      };
    ...
    if (MOBILE) {
      ...
      $("body").keyup(function(e) {
        var tecla = e.keyCode;
        if (tecla == 27) fechar();
        else if ($("#plugins_galeria_imagem").is(":visible") && tecla == 37)
          rotacao("prev");
        else if ($("#plugins_galeria_imagem").is(":visible") && tecla == 39)
          rotacao("next");
      });
  ...
  $.banner = function(options) {
    ...
    if (
      settings.rotacao == true &&
      settings.barra != undefined &&
      $(settings.barra).size() == 1
    )
      ativo_barra = true;
    var ativo_controle = false;
    if (settings.controle != undefined && $(settings.controle).size() == 1)
      ativo_controle = true;
    var ativo_prev = false;
    if (
      MOBILE == false &&
      settings.prev != undefined &&
      $(settings.prev).size() == 1
    )
      ativo_prev = true;
    ...
    function banner(acao) {
      ...
      if ((acao != atual && acao != undefined) || contador == settings.tempo) {
        ...
        if (MOBILE == false) {
          $(settings.id + " " + settings.imagem)
            .stop()
            .animate({ opacity: 0 }, transicao, function() {
              $(this).hide();
            });
          $(settings.id + " " + settings.imagem + ":eq(" + proximo + ")")
            .show()
            .stop()
            .animate({ opacity: 0 }, 0)
          ...
          $(settings.id + " " + settings.imagem + ":eq(" + atual + ")").css(
            "z-index",
            2
          );
          $(settings.id + " " + settings.imagem + ":eq(" + proximo + ")")
        ...
        if (ativo_controle) {
          ...
          $(settings.controle + " li[data-id=" + proximo + "]").addClass(
            "hover"
          );
        }
      } else {
    ...
    function barra() {
      $(settings.barra)
        ...
        .animate(
          { width: "100%" },
          parseInt(parseInt(tempo) + 1 - contador) * 1000
        );
    }
  ...
  $.loading = function(posicao) {
    if (posicao == "show") {
      ...
      $("body").append(
        '<div id="plugins_loading"><span></span><span></span><span></span><span></span></div>'
      );
    } else {
      $("#plugins_loading").remove();
    }
  ...
  $.pagina = function(options) {
        ...
        $("#plugins_pagina_conteudo")
          ...
          .animate(
            ...
            function() {
              ...
              $("#plugins_pagina_conteudo").load(
                botao.attr(settings.attr),
                {
                  ajax: true,
                  dados: dados
                },
    ...
    $("body").on("click", settings.botao, function() {
      ...
      if ($("#plugins_pagina").size() == 1) {
        ...
        $("#plugins_pagina").append(
          '<div id="plugins_pagina_conteudo" class="plugins_pagina_conteudo"></div>'
        );
        if (settings.fechar == true) {
    ...
    $("body").on("click", "#plugins_pagina, #plugins_pagina_conteudo", function(
      e
    ) {
      if ($("#plugins_pagina").attr("data-fecharmascara") == "fechar") {
        if (
          $(e.target).attr("id") == "plugins_pagina" ||
          $(e.target).attr("id") == "plugins_pagina_conteudo"
        )
          fechar();
      }
    });
    ...
    if (MOBILE) {
      window.onhashchange = function() {
        var url_atual = window.location.href;
        if (
          url_atual.indexOf("#paginamobile") == -1 &&
          $("#plugins_pagina").size() == 1
        ) {
          fechar();
        }
      };
  ...
  $.alerta = function(options) {
    ...
    if (
      $("#plugins_alerta_conteudo").size() > 0 &&
      settings.notificacao == ""
    ) {
      fechar_alerta();
      return false;
    }
    ...
    if (settings.notificacao != "") {
      ...
      $("#plugins_alerta").append(
        '<div id="plugins_alerta_conteudo"><div class="plugins_alerta_titulo">' +
          settings.titulo +
          '</div><div class="plugins_alerta_texto">' +
          settings.texto +
          '</div><div class="plugins_alerta_footer"><div class="plugins_alerta_botao plugins_alerta_cancelar">CANCELAR</div><div id="' +
          settings.confirmar +
          '" class="plugins_alerta_botao">CONFIRMAR</div></div></div>'
      );
      abrir_alerta();
    } else {
      $("body").append('<div id="plugins_alerta"></div>');
      $("#plugins_alerta").append(
        '<div id="plugins_alerta_conteudo"><div class="plugins_alerta_titulo">' +
          ...
          '</div><div class="plugins_alerta_texto">' +
          settings.texto +
          '</div><div class="plugins_alerta_footer"><div class="plugins_alerta_botao">OK</div></div></div>'
      );
      abrir_alerta();
    }
    ...
    function abrir_notificacao() {
      ...
      if ($("#plugins_alerta_notificacao").size() == 1) {
        $("#plugins_alerta_notificacao")
          .stop()
          .css("display", "flex")
          .animate({ top: -100, opacity: 0 }, 300, function() {
            $("#plugins_alerta_notificacao").remove();
            $("body").append(
              '<div id="plugins_alerta_notificacao" data-temporizador="' +
                numero +
                '"><div class="plugins_alerta_texto">' +
                settings.notificacao +
                '</div><div class="plugins_alerta_botao">OK</div></div>'
            );
            $("#plugins_alerta_notificacao")
              .stop()
              .css("display", "flex")
      ...
      setTimeout(function() {
        if (
          $(
            "#plugins_alerta_notificacao[data-temporizador=" + numero + "]"
          ).size() == 1
        ) {
          $("#plugins_alerta_notificacao[data-temporizador=" + numero + "]")
            .stop()
    ...
    if (MOBILE) {
      window.onhashchange = function() {
        var url_atual = window.location.href;
        if (
          url_atual.indexOf("#alertamobile") == -1 &&
          $("#plugins_alerta_conteudo").size() == 1
        ) {
          fechar_alerta();
        }
      };
    ...
    $("body").on("keydown", function(e) {
      if (
        $("#plugins_alerta").size() > 0 &&
        $("#plugins_alerta").is(":visible") &&
        (e.which == 13 || e.which == 27)
      ) {
        fechar_alerta();
        return false;
      } else if (
        $("#plugins_alerta_notificacao").size() == 1 &&
        $("#plugins_alerta_notificacao").is(":visible") &&
        (e.which == 13 || e.which == 27)
      ) {
        fechar_notificacao();
        return false;
    ...
    $("body").on(
      "click",
      "#plugins_alerta_conteudo, #plugins_alerta_notificacao",
      function(e) {
        e.stopPropagation();
      }
    );
  ...
  $.mascara = function() {
    ...
    $("body").on("blur", "input[data-mascara=data]", function() {
      ...
      if (
        ((!er.exec($(this).val()) && !er2.exec($(this).val())) ||
          ($(this).val() == "00/00/0000" || $(this).val() == "0000-00-00")) &&
        $(this).val() != ""
      )
    ...
    $("body").on("blur", "input[data-mascara=hora]", function() {
      ...
      if (
        (!er.exec(valor) && valor != "") ||
        (valor.split(":")[0] > 23 || valor.split(":")[1] > 59)
      )
        $(this).addClass("input_erro");
    });
    ...
    $("body").on("keydown", "*[data-mascara]", function(e) {
      ...
      if (tipo == "numero") {
        if (
          $.inArray(key_codigo, key_geral) != -1 ||
          ((e.ctrlKey || e.metaKey) && $.inArray(key_codigo, key_ctrl) != -1)
        ) {
          return true;
        ...
        if (
          ...
          $.inArray(key_codigo, key_numero) != -1 ||
          key_codigo == 32
        ) {
          return true;
        } else {
        ...
        if (
          ...
          if (valor.length == 2) $(this).val(valor + "/");
            ...
            (valor.length == 10 && data_tipo == "br") ||
            (valor.length == 10 && data_tipo == "en" && valor[0] > 0)
          )
            return false;
        ...
        if (
          ...
          else if (valor.length == 16) return false;
...
!(function(a) {
  a.browser ||
    ...
    (a.browser.webkit = /webkit/.test(navigator.userAgent.toLowerCase())),
    (a.browser.opera = /opera/.test(navigator.userAgent.toLowerCase())),
    ...
    destroy: function() {
      ...
      return (
        ...
        a.browser.msie
          ? (this.onpaste = null)
          : a.browser.mozilla &&
            this.removeEventListener("input", blurEvent, !1),
        this
      );
    },
    mask: function() {
    ...
    init: function(b) {
      return (
        ...
        this.each(function() {
          ...
          function g(b) {
            b = b || window.event;
            var g = b.which || b.charCode || b.keyCode;
            if (void 0 == g) return !1;
            if (g < 48 || g > 57)
              return 45 == g
                ? (e(), c.val(q(c)), !1)
                : 43 == g
                ? (e(), c.val(c.val().replace("-", "")), !1)
                : 13 == g || 9 == g
                ? (d && (f(), a(this).change()), !0)
                : !(
                    !a.browser.mozilla ||
            ...
            if (c.val().length >= c.attr("maxlength")) return !1;
            k(b);
            ...
            return (
              (i.value =
                i.value.substring(0, m) +
                h +
                i.value.substring(n, i.value.length)),
              l(i, m + 1),
              e(),
              !1
            );
          ...
          function h(b) {
            ...
            return 8 == g
              ? (k(b),
                j == m
                  ? ((h.value =
                      h.value.substring(0, j - 1) +
                      h.value.substring(m, h.value.length)),
                    (j -= 1))
                  : (h.value =
                      h.value.substring(0, j) +
                      h.value.substring(m, h.value.length)),
                l(h, j),
                e(),
                !1)
              ...
              : (46 != g && 63272 != g) ||
                ...
                h.selectionStart == h.selectionEnd
                  ? (h.value =
                      h.value.substring(0, j) +
                      h.value.substring(m + 1, h.value.length))
                  : (h.value =
                      h.value.substring(0, j) +
                      h.value.substring(m, h.value.length)),
                l(h, j),
          ...
          function j(d) {
            a.browser.msie && g(d),
              "" == c.val() || c.val() == p(o()) || c.val() == b.symbol
                ? b.allowZero
                  ? b.symbolStay
                    ? c.val(p(o()))
                    : c.val(o())
                  : c.val("")
                : b.symbolStay
                ? b.symbolStay && c.val() == b.symbol && c.val(p(o()))
                : c.val(c.val().replace(b.symbol, ""));
          }
          function k(a) {
          ...
          function n(a) {
            ...
            if (
              (0 != d &&
                "-" == a.charAt(0) &&
                ((a = a.replace("-", "")), b.allowNegative && (g = "-")),
              0 == d)
            ) {
              if (!b.defaultZero) return f;
              f = "0.00";
            }
            for (
              var h = 0;
              h < d && ("0" == a.charAt(h) || a.charAt(h) == b.decimal);
              h++
            );
            for (; h < d; h++)
              c.indexOf(a.charAt(h)) != -1 && (e += a.charAt(h));
            var i = parseFloat(e);
            (i = isNaN(i) ? 0 : i / Math.pow(10, b.precision)),
              (f = i.toFixed(b.precision)),
              (h = 0 == b.precision ? 0 : 1);
            var j,
              k = (f = f.split("."))[h].substr(0, b.precision);
            for (j = (f = f[0]).length; (j -= 3) >= 1; )
              f = f.substr(0, j) + b.thousands + f.substr(j);
            return p(
              b.precision > 0
                ? g +
                    f +
                    b.decimal +
                    k +
                    Array(b.precision + 1 - k.length).join(0)
                : g + f
            );
          }
          function o() {
            ...
            return a
              .toFixed(b.precision)
              .replace(new RegExp("\\.", "g"), b.decimal);
          }
          function p(a) {
            if (b.showSymbol) {
              var c = "";
              0 != a.length &&
                "-" == a.charAt(0) &&
                ((a = a.replace("-", "")), (c = "-")),
                a.substr(0, b.symbol.length) != b.symbol &&
                  (a = c + b.symbol + a);
            }
            return a;
          }
          ...
          function r(b, c) {
            return (
              a(b).each(function(a, b) {
                if (b.setSelectionRange) b.focus(), b.setSelectionRange(c, c);
              ...
              "number" == typeof a.selectionEnd
                ...
                : ((e = document.selection.createRange()),
                  e &&
                    ...
                    (h = a.createTextRange()),
                    h.collapse(!1),
                    f.compareEndPoints("StartToEnd", h) > -1
                      ? (b = c = g)
                      : ((b = -f.moveStart("character", -g)),
                        (b += d.slice(0, b).split("\n").length - 1),
                        f.compareEndPoints("EndToEnd", h) > -1
                          ? (c = g)
                          : ((c = -f.moveEnd("character", -g)),
                            (c += d.slice(0, c).split("\n").length - 1))))),
              { start: b, end: c }
            );
          }
          ...
          c.attr("readonly") ||
            (c.unbind(".maskMoney"),
            c.bind("keypress.maskMoney", g),
            c.bind("keydown.maskMoney", h),
            c.bind("blur.maskMoney", j),
            c.bind("focus.maskMoney", i),
            c.bind("mask.maskMoney", m));
        })
      );
    }
  };
  a.fn.maskMoney = function(c) {
    return b[c]
      ...
      : "object" != typeof c && c
      ? void a.error("Method " + c + " does not exist on jQuery.tooltip")
...
$(function(){
    ...
    **/
    ...
    $('body').on('click', '.bloco_grafico_erro i', function(){
        $(this).closest('.bloco_grafico_erro').remove();
    });
    ...
    **/
    $('body').on('change', '.but_change_atualizar', function(){
        ...
        var nome = $(this).attr('name') || $(this).attr('data-name') || $('*[name]', this).attr('name');
        var valor = $(this).val() || $('*[name]', this).val();

        if(SISTEMA == 'producao'){
            $.loading('show');
        }
        $.post(PAINEL+'/post-atualizar-unico', {
            id:id,
            app:app,
            nome:nome,
            valor:valor
            ...
            $.loading('hide');
            if(historico == 1) historico_add(app, cod, '', '', undefined, 6, false);
        });
    });
    ...
    **/
    $('.bloco_arquivo_lista').on('focus', '.lista input', function(){
        $(this).closest('li.arquivo').addClass('hover');
    });
    $('.bloco_arquivo_lista').on('blur', '.lista input', function(){
        $(this).closest('li.arquivo').removeClass('hover');
    });
    ...
    $('.bloco_arquivo_lista').on('click', 'li.arquivo .but_arquivo_deletar', function(){
    ...
    $('body').on('click', '#but_arquivo_deletar', function(){
        ...
        $.post(PAINEL+'/post-deletar', {
			app:app,
			dados:dados
		}, function(){
            li.remove();
            $.alerta({notificacao:'Registro deletado com sucesso!'});
            if($('li.arquivo', bloco).size() == 0) $('.lista', bloco).html('<li class="zero">SEM ARQUIVOS NO MOMENTO</li>');
        });
    });
    ...
    var upload_arquivo_lista = function(resposta, bloco){
        if(resposta.erro == true){
            ...
            if($('.lista li.zero', bloco).size() > 0) $('.lista li.zero', bloco).remove();
            $('.lista', bloco).append('<li class="arquivo" style="background-image: url('+LINK+'/images/ext/file.png)"><a class="download" download="'+resposta.arquivo+'" href="'+resposta.link+'"><i data-font="&#xf0ed;"></i></a><a class="deletar but_arquivo_deletar" data-id="'+resposta.id+'" href="#deletar"><i data-font="&#xe808;"></i></a><input type="text" data-name="titulo" class="but_change_atualizar" data-id="'+resposta.id+'" data-app="arquivo" placeholder="Nome do arquivo" value="'+resposta.arquivo+'"><div class="nome">'+resposta.arquivo+'</div></li>');
        }
    ...
    var upload_imagem = function(resposta, bloco, retorno){
        if(resposta.erro == true){
            ...
            if(retorno == 'img'){
                ...
                $('figure', bloco).css('background-image', 'url('+resposta.link+')');
            }
    ...
    **/
    var upload_imagem_lista = function(resposta, bloco, retorno){
        if(resposta.erro == true){
            ...
            if(retorno == 'img'){
                $('.bloco_imagem_lista_conteudo', bloco).append('<figure data-id="'+resposta.id+'"><img src="'+resposta.link+'" /><i class="fechar" data-font="&#xe813;" data-ajuda="Remover imagem" data-id="'+resposta.id+'"></i><input type="text" placeholder="Nome da imagem" /></figure>');
            }else if(retorno == 'css'){
                $('.bloco_imagem_lista_conteudo', bloco).append('<figure style="background-image: url('+resposta.link+')" data-id="'+resposta.id+'"><i class="fechar" data-font="&#xe813;" data-ajuda="Remover imagem"></i><input type="text" placeholder="Nome da imagem" /></figure>');
            }
        }
        $('input[type=file]', bloco).val('');
        $.loading('hide');
    ...
    $('body').on('click', '#but_imagem_lista_remover', function(){
        ...
        $.post(PAINEL+'/post-upload-imagem-lista-deletar', {id:id}, function(resposta){
            if(resposta.erro == false){
                bloco.remove();
                $.alerta({notificacao: 'Imagem deletada com sucesso.'});
                if($('figure', bloco_imagem).size() == 0) bloco_imagem.html('<div class="zero_geral">SEM IMAGENS ENVIADAS ATÉ O MOMENTO</div>');
            }else {
                $.alerta({
                    titulo: resposta.titulo,
                    texto: resposta.texto
    ...
    **/
    var tag_add = function(bloco){
        ...
        if(valor != ''){
            ...
            $('.bloco_tag_input_botao_ul', bloco).append('<li class="bloco_tag_input_li"><span class="bloco_tag_input_span">'+valor+'</span><input type="hidden"  data-array="1" name="'+nome+'" value="'+valor+'"><i class="bloco_tag_input_deletar" data-ajuda="De 2 cliques para apagar" data-font="&#xe813;"></i></li>');
        }
    };
    $('body').on('dblclick', '.bloco_tag_input .bloco_tag_input_deletar', function(){
        $('#plugins_ajuda').remove();
        $(this).closest('.bloco_tag_input_li').remove();
    });
    $('body').on('click', '.bloco_tag_input .bloco_tag_input_add', function(){
        tag_add($(this).closest('.bloco_tag_input'));
    });
    $('body').on('keyup', '.bloco_tag_input .bloco_tag_input_botao .bloco_tag_input_input', function(e){
        if(e.keyCode == 13){
            tag_add($(this).closest('.bloco_tag_input'));
        }
    });
    ...
    **/
    ...
    var bloco_lista_input_busca = function(bloco){
        ...
        if(link != ''){
            $.post(link,{busca:valor}, function(resposta){
                if(resposta.erro == false){
            ...
            $('.bloco_lista_input_input', bloco).val('');
			$('.bloco_lista_input_ul', bloco).append('<li class="bloco_lista_input_li"><input type="hidden" name="'+nome+'" data-array="1" value="'+valor+'"><div class="bloco_lista_input_nome">'+valor+'</div><i class="bloco_lista_input_remover" data-ajuda="Remover da lista" data-font="&#xe813;"></i></li>');
        }
    };
    ...
    $('body').on('click', '#but_lista_input_remover', function(){
        ...
        if($('.bloco_lista_input_li', bloco_lista_input_remove_bloco).length == 0){
            $('.bloco_lista_input_ul', bloco_lista_input_remove_bloco).html('<li class="bloco_lista_input_zero">SEM USUÁRIO VINCULADO</li>');
        }
        return false;
    });
    ...
    **/
    $('body').on('focus', '.bloco_video input', function(){
        $(this).closest('.bloco_video').addClass('hover');
    });
    $('body').on('blur', '.bloco_video input', function(){
        $(this).closest('.bloco_video').removeClass('hover');
    });
    $('body').on('change', '.bloco_video input', function(){
        ...
        if(valor.indexOf('youtube.com') != -1){
            ...
            $('.responsivo', bloco).html('<iframe src="https://www.youtube.com/embed/'+explode[explode.length-1]+'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
            bloco.addClass('ativo');
        }else if(valor.indexOf('vimeo.com') != -1){
            explode = valor.split('/');
            $('.responsivo', bloco).html('<iframe src="https://player.vimeo.com/video/'+explode[explode.length-1]+'?title=0&byline=0&portrait=0" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>');
            bloco.addClass('ativo');
        }else {
	...
	**/
	...
	var historico_add = function(app, cod, bloco, booleano, volta, tipo, fechar){
		...
		$('#bloco_historico_add').css('display', 'flex').stop()
		.animate({opacity:0}, 0)
		.animate({opacity:1}, 300);
		...
		if(booleano == undefined || booleano == ""){
            ...
            $('#bloco_historico_add .historico_booleano .historico_booleano_texto').html(booleano);
		...
		if(historico_app != undefined) $('#bloco_historico_add input[name=app]').val(historico_app);
		if(historico_cod != undefined) $('#bloco_historico_add input[name=cod]').val(historico_cod);
		if(bloco != undefined) $('#bloco_historico_add input[name=bloco]').val(bloco);
		if(volta != undefined) $('#bloco_historico_add input[name=volta]').val(volta);
		if(tipo != undefined){
			$('#bloco_historico_add select[name=tipo]').val(tipo);
			$('#bloco_historico_add .tipo li').removeClass('hover');
			$('#bloco_historico_add .tipo li[data-tipo='+tipo+']').addClass('hover');
		}
	};
	...
	var historico_fechar = function(acao){
		...
		if(acao == true && bloco != '' && $('#'+bloco).size() == 1){
			if($('#'+bloco+' li.zero').size() > 0) $('#'+bloco+' li.zero').remove();
			$('#'+bloco).prepend('<li class="lista"><i class="'+icone+'"></i><div class="dados_lista"><span class="nome">'+EQUIPE_NOME+'</span> <span class="data">Agora</span><p>'+texto+'</p></div></li>');
			$.loading('hide');
		}else if(volta != ''){
	...
	$('body').on('click', '#bloco_historico_add button', function(){
		...
		var tipo = $('#bloco_historico_add select[name=tipo]').val() || '';
		var texto = $('#bloco_historico_add textarea[name=texto]').val() || '';
		var booleano = $('#bloco_historico_add input[type=checkbox][name=booleano]:checked').val() || '';
		...
		$.post(link, {
			...
			if(resposta.erro == false){
				...
				$.alerta({notificacao:'Erro ao salvar, por favor, repotar a equipe técnica.'});
				$.loading('hide');
			}
		}, 'json');
		return false;
	...
	var historico_lista_fechar = function(){
		$('#bloco_historico_abrir').stop()
		.animate({opacity:0}, 300, function(){
			$(this).hide();
			$('#bloco_historico_abrir ul').html('<li class="loading">CARREGANDO HISTÓRICO</li>');
			$('body').css('overflow-y', 'auto');
		});
	};
	...
	$('.but_visualizar_historico').click(function(){
		...
		$('#bloco_historico_abrir').stop().css('display', 'flex')
		.animate({opacity:0}, 0)
		.animate({opacity:1}, 300, function(){
			$('#bloco_historico_abrir ul').html('<li class="loading">CARREGANDO HISTÓRICO</li>');
			$('#bloco_historico_abrir ul').load(PAINEL+'/historico_lista', {
				pagina:historico_pagina,
				cod:cod,
				app:app,
			}, function(resposta){
				$.loading('hide');
				historico_quantidade = $('#bloco_historico_abrir ul span:first').attr('data-pagina');
	...
	$('#bloco_historico_abrir').click(function(e){
		if($(e.target).attr('id') == 'bloco_historico_abrir') historico_lista_fechar();
	});
	...
	**/
    var form_dados = function(form, falso){
		...
		$('*[name]', form).each(function(i){
			...
			}else if($(this).attr('data-mascara') == 'datahora'){                
                dados[nome] = $(this).val() + ":00";
			}else {
                var valor;
                if(falso == true){
    ...
    **/
    $('body').on('click', '.but_previsualizacao', function(){
        ...
        if($('#bloco_previsualizacao').size() > 0) $('#bloco_previsualizacao').remove();
        $('body').append('<div id="bloco_previsualizacao"><i class="fechar" id="but_previsualizacao_fechar"></i><div class="conteudo"></div></div>');
        ...
        $('body').css('overflow-y', 'hidden');
        $('#bloco_previsualizacao .conteudo').load(PAINEL+'/pre-visualizacao/'+$('#form_app_geral').val(), {
            dados:dados
        }, function(){
            $.loading('hide');
            $('#bloco_previsualizacao').stop().show()
            .animate({'opacity': 0}, 0)
            .animate({'opacity': 1}, 300);
        });
    });
    $('body').on('click', '#but_previsualizacao_fechar, #bloco_previsualizacao', function(e){
        if($(e.target).attr('id') == 'but_previsualizacao_fechar' || $(e.target).attr('id') == 'bloco_previsualizacao'){
            $('body').css('overflow-y', 'auto');
            $('#bloco_previsualizacao').stop()
            .animate({opacity: 0}, 300, function(){
                $(this).remove();
            });
        }
	...
	$('body').on('click', '#but_add_geral', function(){
		...
		$.post(link, {
			...
			if(resposta.erro == false){
                ...
                var mensagem_ok;
				if(dados.id == undefined) mensagem_ok = 'Dados cadastrado com sucesso.';
				else mensagem_ok = 'Dados atualizado com sucesso.';

				if($('#plugins_pagina').size() == 1) $('#plugins_pagina').click();

				if(historico == 1) historico_add(app, dados.cod, '', booleano, volta, 5, true);
				...
				$.loading('hide');
				$.alerta({notificacao:'Erro no envio, por favor, informar suporte.'});
			}
			$.loading('hide');
		}, 'json');
		return false;
	...
	**/
	$('.but_deletar_lista').click(function(){
		$.alerta({
			titulo:'CUIDADO!',
			texto:'Você irá deletar os registros selecionados! Essa ação não poderá ser desfeita, tem certeza que deseja continuar?',
	...
	$('body').on('click', '#but_deletar_lista', function(){
		...
		$.post(PAINEL+'/post-deletar', {
			...
			if($('#bloco_lista_geral li.li').size() == 0){
				$('#bloco_lista_geral').html('<div class="zero"><i data-font="&#xe801;"></i><span>SEM DADOS NO MOMENTO</span></div>');
	...
	**/
	$('.bloco_admin span').click(function(e){
        $(this).closest('.bloco_admin').addClass('bloco_esperando_autorizacao');
    	var tipo = $(this).attr('data-admin') || 3;
		if(tipo == 1){
			$('#bloco_autorizacao input[name=login]').show();
			$('#bloco_autorizacao input[name=login]').focus();
            $('#bloco_autorizacao input[name=login]').attr('placeholder', 'Login do administrador');
            $('#bloco_autorizacao form .input_password_geral .bloco_password input').attr('placeholder', 'Senha do administrador');
		}else if(tipo == 2){
            $('#bloco_autorizacao input[name=login]').show();
            $('#bloco_autorizacao input[name=login]').focus();
            $('#bloco_autorizacao input[name=login]').attr('placeholder', 'Login do gerente');
            $('#bloco_autorizacao form .input_password_geral .bloco_password input').attr('placeholder', 'Senha do gerente');
        }else if(tipo == 3){
            $('#bloco_autorizacao input[name=login]').hide();
            $('#bloco_autorizacao input[name=password]').focus();
            $('#bloco_autorizacao form .input_password_geral .bloco_password input').attr('placeholder', 'Digite sua senha');
		}
    	$('#bloco_autorizacao input[name=tipo]').val(tipo);

		$('#bloco_autorizacao').css('display', 'flex').stop()
		.animate({opacity:0}, 0)
		.animate({opacity:1}, 300, function(){
            if(tipo == 1 || tipo == 2){
                $('#bloco_autorizacao input[name=login]').focus();
	...
	var autorizacao_fechar = function(){
        $('.bloco_esperando_autorizacao').removeClass('bloco_esperando_autorizacao');
		$('#bloco_autorizacao').stop()
		.animate({opacity:0}, 300, function(){
			$(this).hide();
    ...
    **/
    ...
    var endereco_abrir = function(){
        $('#bloco_endereco').stop().css('display', 'flex')
		.animate({opacity:0}, 0)
		.animate({opacity:1}, 300);

        var icone = LINK+'/images/painel/mapa.png';
        var latitude = $('#bloco_endereco .bloco_mapa input[name=latitude]').val();
        var longitude = $('#bloco_endereco .bloco_mapa input[name=longitude]').val();
    ...
    $('body').on('click', '.but_add_endereco', function(){
		...
		$('#bloco_endereco .bloco_conteudo').load(PAINEL+'/endereco/add', {
			cod:cod,
			app:app,
			local:local,
			bloco:bloco
    ...
    $('body').on('click', '.but_editar_endereco', function(){
		...
		var app = $(this).attr('data-app');
		var local = $(this).attr('data-local');
		var bloco = '#'+$(this).closest('.form_lista').attr('id');

		$('#bloco_endereco .bloco_conteudo').load(PAINEL+'/endereco/editar', {
			id:id,
			app:app,
			bloco:bloco,
            local:local
    ...
    $('#bloco_endereco').on('change', 'input[name=cep]', function(){
        ...
        $.post(PAINEL+'/api-cep', {
            ...
            if(resposta.erro == false){
                ...
                if(resposta.logradouro == '') $('input[name=logradouro]', bloco).focus();
        ...
        if(latitude == '' || longitude == ''){
            $.post(PAINEL+'/api-mapa', {
                local:valor
            }, function(resposta){
                if(resposta.erro == false){
                    abrir_mapa('bloco_endereco_mapa', resposta.latitude, resposta.longitude, LINK+'/images/painel/mapa.png');

                    $('#bloco_endereco .bloco_mapa input[name=latitude]').val(resposta.latitude);
                    $('#bloco_endereco .bloco_mapa input[name=longitude]').val(resposta.longitude);
                    $('#bloco_endereco_como_chegar').attr('href', 'https://www.google.com.br/maps/dir//'+resposta.latitude+', '+resposta.longitude);
                }else {
                    abrir_mapa('bloco_endereco_mapa', '-15.7941569', '-47.8825289', '');
                }
            }, 'json');
        }else {
            if($('input[name=cep]', bloco) != '') $('#bloco_endereco .bloco_mapa i.atualizar').addClass('atencao');
        }
        return false;
    });
        ...
        if(local != ''){
            $.post(PAINEL+'/api-mapa', {
                ...
                if(resposta.erro == false){
                    ...
                    abrir_mapa('bloco_endereco_mapa', '-15.7941569', '-47.8825289', '');
                    $('#bloco_endereco_como_chegar').attr('href', 'https://www.google.com.br/maps/dir//');
                }
            }, 'json');
        }
    });
    // Atualiza pela latitude e longitude
    $('#bloco_endereco').on('change', 'input[name=latitude], input[name=longitude]', function(){
        var latitude = $('#bloco_endereco input[name=latitude]').val();
        var longitude = $('#bloco_endereco input[name=longitude]').val();

        if(latitude != '' && longitude != ''){
            abrir_mapa('bloco_endereco_mapa', latitude, longitude, LINK+'/images/painel/mapa.png');
            $('#bloco_endereco_como_chegar').attr('href', 'https://www.google.com.br/maps/dir//'+latitude+', '+longitude);
        }
    });
    // Cancela o ENTER
    $('#bloco_endereco').on('keydown', 'input[name=latitude], input[name=longitude]', function(e){
    ...
    $('#bloco_endereco').on('click', '#but_endereco_salvar', function(){
		...
		$.post(link, {
			...
			if(resposta.erro == false){
				$('.zero', bloco).remove();
				if(resposta.id){
					$('ul', bloco).append('<li class="lista"><span class="valor">'+nome+' - '+logradouro+'</span><i data-font="&#xe807;" data-local="'+local+'" data-id="'+resposta.id+'" data-app="'+app+'" class="editar but_editar_endereco"></i></li>');
				}else {
	...
	$('body').on('click', '#but_endereco_confirmar_deletar', function(){
		...
		$.post(PAINEL+'/post-endereco-deletar', {id:id}, function(resposta){
			if(resposta.erro == false){
				$(bloco+' .but_editar_endereco[data-id='+id+']').closest('li').remove();
				$('#bloco_endereco').click();
				if($(bloco+' li.lista').size() == 0) $(bloco+' .bloco_conteudo').append('<li class="zero">SEM DADOS NO MOMENTO</li>');
			}else {
				$.alerta({
					titulo:resposta.titulo,
					texto:resposta.texto
	...
	**/
	var mensagem_abrir = function(){
		$('#bloco_contato').stop().css('display', 'flex')
		.animate({opacity:0}, 0)
		.animate({opacity:1}, 300);
	};
	...
	$('body').on('click', '.but_add_contato', function(){
		...
		$('#bloco_contato .bloco_conteudo').load(PAINEL+'/contato/add', {
			cod:cod,
			app:app,
			local:local,
			bloco:bloco
	...
	$('body').on('click', '.but_editar_contato', function(){
		...
		$('#bloco_contato .bloco_conteudo').load(PAINEL+'/contato/editar', {
			id:id,
			app:app,
			bloco:bloco,
            local:local
	...
	$('#bloco_contato').on('click', '#but_contato_salvar', function(){
		...
		if(tipo == 1){
            ...
            nome_texto = $('#bloco_contato select[name=nome_telefone] option:selected').text();
		}else {
			nome = $('#bloco_contato select[name=nome_email]').val();
			valor = $('#bloco_contato input[name=valor_email]').val();
			operadora = false;
            nome_texto = $('#bloco_contato select[name=nome_email] option:selected').text();
		}
		...
		$.post(link, {
			...
			if(resposta.erro == false){
				$('.zero', bloco).remove();
				if(resposta.id){
					$('ul', bloco).append('<li class="lista"><span class="valor">'+contato+' - '+valor+'</span><i data-font="&#xe807;" data-local="'+local+'" data-id="'+resposta.id+'" data-app="'+app+'" class="editar but_editar_contato"></i></li>');
				}else {
					var li = $(bloco+' li i[data-id='+id+']').closest('li');
	...
	$('body').on('click', '#but_contato_confirmar_deletar', function(){
		...
		$.post(PAINEL+'/post-contato-deletar', {id:id}, function(resposta){
			if(resposta.erro == false){
				$(bloco+' .but_editar_contato[data-id='+id+']').closest('li').remove();
				$('#bloco_contato').click();
				if($(bloco+' li.lista').size() == 0) $(bloco+' .bloco_conteudo').append('<li class="zero">SEM CONTATO NO MOMENTO</li>');
			}else {
				$.alerta({
					titulo:resposta.titulo,
					texto:resposta.texto
	...
	$('#bloco_contato').on('change', '#input_contato_nome_email, #input_contato_nome_telefone', function(){
    ...
    **/
    $('body').on('click', '#bloco_agenda_add button', function(){
        ...
        $.post(link, {dados:dados}, function(resposta){
            if(resposta.erro == false){
                ...
                $(id).append('<li class="lista"><span class="valor">'+dados.titulo+valor+'</span><i data-font="'+icone+'" data-href="'+PAINEL+'/incorporar/agenda_calendario/'+visualizar_tipo+'/par/'+resposta.cod+'" data-ajuda="'+ajuda+'" class="editar but_bloqueado_ajax"></i></li>');
    ...
    // Faz a ligação
    var janela_ligacao;
    $('body').on('click', '#bloco_agenda_realizar .but_fazer_ligacao', function(){
        $('#bloco_ajuda').remove();
        if($('#bloco_agenda_realizar .conteudo').hasClass('ligando')){
            ...
            $('#bloco_agenda_realizar textarea').focus();
            $('#bloco_agenda_realizar .ligar').attr('data-ajuda', 'Encerrar ligação');

            janela_ligacao = window.open($(this).attr('href'), 'janela_ligacao', 'width=1, height=1');
        }
        return false;
    });
    // Cancela a ligação
    $('body').on('click', '#bloco_agenda_realizar .but_cancelar_ligacao', function(){
        $('#bloco_agenda_realizar .header_voltar').remove();
    ...
    // Muda o status
    $('body').on('click', '#bloco_agenda_realizar .but_agenda_status', function(){
    ...
    $('body').on('click', '#bloco_agenda_realizar input[name=data], #bloco_agenda_realizar input[name=hora]', function(){
        $(this).select();
    });
    // Salva histórico
    $('body').on('click', '#bloco_agenda_realizar button', function(){
        ...
        $.post(PAINEL+'/incorporar/agenda_calendario/post-historico', {
            data:data,
            hora:hora,
            status:status,
            cod:cod,
	...
	**/
	...
	$('.but_social_abrir').click(function(){
		$('body').css('overflow-y', 'hidden');
		$('#bloco_social').stop().css('display', 'flex')
		.animate({opacity:0}, 0)
		.animate({opacity:1}, 300);
		return false;
	});
	...
	**/
	...
	if($('.header_principal_animacao').size() > 0) header_principal = $('.header_principal_animacao').offset().top;
	else header_principal = 0;
	if(header_principal) header_scroll($(document).scrollTop(), header_principal);
	...
	**/
	$('body').on('change', '.bloco_checkbox_todos input[data-name=todos]', function(){
	...
	$('body').on('change', '.bloco_checkbox_todos input[data-name!=todos]', function(){
		...
		if(conta == quantidade) $('input[data-name=todos]', bloco).prop('checked', true);
    ...
    **/
    $('.clonar_parceiro').click(function(){
        ...
        //$.loading('show');
        $.post($('#PAINEL').val()+'/incorporar/convenio_parceiro/clonar', {
            cod:cod
        }, function(resposta){
            if(resposta.erro == false){
                $.alerta({
                    ...
                    href: $('#PAINEL').val() + "/visualizar/convenio_parceiro/" + resposta.cod
                });
            }else if(resposta.erro == true){
                $.alerta({
                    titulo:resposta.titulo,
