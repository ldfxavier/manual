...
class PainelModel extends Model
{
	...
	{
		if (isset($dados['url'])) :
			if (isset($dados['titulo_grande'])) :
				$dados['url'] = Converter::acento($dados['titulo_grande'], '-');
	...
	public function p_deletar($app, $dados)
	{
		...
		if ($dados) foreach ($dados as $id) $Model->delete('id', $id);
	}
