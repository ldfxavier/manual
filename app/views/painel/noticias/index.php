...
<div id="bloco_{{$_app}}" class="app_geral_lista">
    <header class="header_principal header_principal_animacao header_principal_absolute">
		...
		<?php if(!empty($where)): ?>
        <i data-font="&#xe813;" class="icones but_remove_filtro" data-ajuda="REMOVER FILTRO"><a href="{{PAINEL}}/post-busca-remover/{{$_app}}"></a></i>
		<?php endif; ?>
        <i data-font="&#xe805;" class="icones but_bloco_ajax" data-width="600" data-href="{{PAINEL}}/busca/{{$_app}}" data-ajuda="FILTRAR DADOS"></i>
		...
		<ul class="ul_lista lista" id="bloco_lista_geral" data-app="<?php echo $_app ?>">
			...
			<li class="li <?php echo ($selecao) ? 'but_visualizar_geral' : '' ?>" data-id="<?php echo $r->id ?>">
				<a href="{{PAINEL}}/editar/{{$_app}}/<?php echo $r->cod ?>">
					...
					<div class="td"><?php echo P::filtro($r->titulo, 'Sem título') ?></div>
					<div class="td"><?php echo P::filtro($r->data->postagem_inicio->br, 'Sem data') ?></div>
					<div class="td status"><span style="background-color: <?php echo $r->status->cor ?>" data-ajuda="<?php echo $r->status->texto ?>"></span></div>
				</a>
			</li>
			<?php
					endforeach;
